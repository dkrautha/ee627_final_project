...
output = pd.read_csv('data/output1.txt', header=None, sep='|',
                     names=["userID", "trackID", "albumRating", "artistRating"])
# df.printSchema()
# %%
# read test2_new.txt with schema ["artistID", "trackID", "label"]
test2 = pd.read_csv('data/test2_new.txt', header=None, sep='|', names=["userID", "trackID", "recommendation"])
...
numericCols = ['albumRating',
               'artistRating']  # TODO: Add Genres Later, 'num_genre_ratings', 'max', 'min', 'mean', 'variance', 'median']
stages = []
...
test_dataframe = pd.read_csv(output_for_df_training, header=0, sep=',', names=["userID", "trackID"])
...
sort_predictions = predictions.select('userID', 'trackID', 'probability',
                                      'rawPrediction', 'prediction'
                                      ).sort(col("userID").asc(), col("probability").desc())
# %%
import os
...
dt = DecisionTreeClassifier(featuresCol='features', labelCol='recommendation', maxDepth=3)
dtModel = dt.fit(train_data)
predictions = dtModel.transform(test_dataframe)
sort_predictions = predictions.select('userID', 'trackID', 'probability',
                                      'rawPrediction', 'prediction').sort(col("userID").asc(),
...
sort_predictions = predictions.select('userID', 'trackID', 'probability', 'rawPrediction', 'prediction').sort(col("userID").asc(), col("probability").desc())
# %%
# sort_predictions.show(6)
write_to_submission(sort_predictions)
# %%
